...
const { matchStreets } = require('../src/state/matching/matcher_kdtree');
const { fakeSeg } = require('../src/utils/fakeSeg');
const { computeStreetConfidence } = require('../src/utils/debug/debugConfidence');
const { runMatcherAzure } = require('../src/state/matching/matcher_ai');
...
describe("Real-world performance tests", () => {
    ...
    // test("matchStreets using AI", async () => {
    //     const start = performance.now();
    //     console.log("Starting AI matcher test...");

    //     await runMatcherAzure(osmStreets, stravaActivities);

    //     const end = performance.now();
    //     console.log("AI matcher (3000 streets, 667 acts):", (end - start).toFixed(2), "ms");
    // });
